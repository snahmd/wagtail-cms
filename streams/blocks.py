...
class Link(blocks.StructBlock):
  ...
  def clean(self,value):
    ...
    if internal_page and external_link:
      errors["internal_page"] = ErrorList(["Both of these fields connot be filled. Please select or enter only one option"])
      errors["external_link"] = ErrorList(["Both of these fields connot be filled. Please select or enter only one option"])
...
class ImageAndTextBlock(blocks.StructBlock):
  image = ImageChooserBlock(
    help_text="Image the automatically cropped to 786px by 552px",
  )
...
class PricingTableBlock(TableBlock):
  ...
  class Meta:
    ...
    label = "Pricing Table"
    icon = "table"
    help_text = "Your pricing tables should always have 4 colums"